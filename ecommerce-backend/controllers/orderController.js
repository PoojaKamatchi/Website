...
export const createOrder = async (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ message: "Payment screenshot required" });
    ...
    const uploadedScreenshot = await cloudinary.uploader.upload(req.file.path, {
    ...
    const order = await Order.create({
      ...
      paymentScreenshot: uploadedScreenshot.secure_url,
    });
